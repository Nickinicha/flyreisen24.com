...
(function() {
    ...
    function switchLanguage(langCode, label) {
        ...
        document.querySelectorAll('.lang-dropdown-menu a').forEach(function (link) {
            const onclick = link.getAttribute('onclick') || '';
            link.classList.toggle('active', onclick.indexOf("'" + langCode + "'") !== -1);
        });
        ...
        if (targetUrl && targetUrl !== path && targetUrl !== path + window.location.search + window.location.hash) {
    ...
    function ensureContactWidgetStyles() {
        ...
        style.textContent = `
.contact-widget { bottom: 92px !important; right: 24px !important; z-index: 99999 !important; display: block !important; }
.contact-button {
